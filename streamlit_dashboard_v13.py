...
DASHBOARD_UI_VERSION = "streamlit-dashboard-v15-stable-ux-footer"

_COMPONENT = components.declare_component(
    "student_samed_dashboard_v15",
    path=str(Path(__file__).with_name("dashboard_component")),
)

_PARENT_CSS = r"""
<style id="streamlit-dashboard-v15-parent">
...
def render_dashboard_v13(
    *,
    ...
    event = _COMPONENT(
        ...
        default=None,
        key="student_samed_dashboard_v15",
    )
    ...
    if not token or not action:
        return None
    if st.session_state.get("_dashboard_v15_event_token") == token:
        return None
    st.session_state["_dashboard_v15_event_token"] = token
    return action

...
st.markdown(f'<div class="tip-box">{st.session_state["book_daily_tip"]}</div>', unsafe_allow_html=True)
...
if qtype == "interactive":
    ...
    with top_l:
        ...
        _q_card_html = (
            '<div class="q-card">'
            '<span class="type-chip-interactive">🧮 تمرين حسابي تفاعلي</span>'
            f'<h2>{q["title"]}</h2>'
            f'<p style="font-size:1.15rem;">{q["text"]}</p>'
            f'{figure_html(qid)}'
            f'<div class="qprogress-bg"><div class="qprogress-fill" style="width:{q_progress_pct}%;"></div></div>'
            f'<small>الخطوة {min(current_step_user_at, len(q["steps"]))} من {len(q["steps"])} &nbsp;|&nbsp; '
            f'⏱️ الوقت: {fmt_time(elapsed)}</small>'
            '</div>'
        )
        st.markdown(_q_card_html, unsafe_allow_html=True)
    with top_r:
        st.write("")
...
# ==========================================================
    ...
    with top_l:
        _q_card_html = (
            ...
            '<span class="type-chip-proof">🧠 تمرين مفاهيمي ورمزي</span>'
            f'<h2>{q["title"]}</h2>'
            f'<p style="font-size:1.15rem;">{q["text"]}</p>'
            f'{figure_html(qid)}'
            f'<div class="qprogress-bg"><div class="qprogress-fill" style="width:{q_progress_pct}%;"></div></div>'
